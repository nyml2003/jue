...
4. 同一次 reconcile 中 key 必须唯一，重复 key 直接拒绝，不能进入半更新状态。
5. runtime payload 写入失败时必须停留在 `ACTIVE`，不能把 lifecycle 卡在 `UPDATING`。

当前实现：

- 已有最小真实 DOM item attach / reconcile / clear。
- reconcile payload 支持 `insert`、`remove`、`move`。
- web 层使用 child tree 表达 item 内容，普通 reorder 在 region anchor 内完成。
- 当前 diff 以正确性为先，不代表最终长列表性能模型；长列表仍属于 `VIRTUAL_LIST`。
...
4. replace 必须先确认新 child tree 可以挂载并定位，再提交稳定 region state。

当前实现：

- 已有真实 child tree attach / replace / detach。
- child tree 挂载在 region anchor 内，而不是 append 到父容器末尾。
- example 已通过 `mountedTree.regions.nested(slot)` 走 controller，不再直接调用 runtime-core 状态机。
...
- runtime 已有 `itemCount / windowStart / windowEnd / targetWindowStart / targetWindowEnd` 状态。
- web controller 已有 `attach()`、`updateWindow()`、`clear()`。
- `updateWindow()` 复用固定 cell tree，通过写 cell signals 重绑定内容。
- `packages/examples/web-playground/src/tab-panel.ts` 已把第二个 tab 接到 `VIRTUAL_LIST`，通过 scroll viewport 和 spacer 驱动窗口变化。
- 当前还没有 overscan、动态池策略、item 高度测量和真实长列表 benchmark。
