...
- 节点宿主共享的 `runtime-core` 模型

它不直接定义：

- 模板宿主的 target glue
- 小程序 / Skyline 的 `setData path` 提交细节

对模板宿主，应复用同一套 signal / binding / dependency graph 语义，但由独立 target/backend 消费 target artifact，而不是强行走同一个 `HostNode[] + Blueprint patch` 路径。
...
- `BlockIR` 负责语义表达
- lowering 负责压平和布局优化
- 节点宿主 runtime 只消费 `Blueprint`

模板宿主则应理解成：

`author input -> BlockIR -> lowering -> target artifact -> target glue`

也就是说，`runtime-core` 和 target glue 可以共享同一套依赖语义，但不是同一份执行 contract。
...
对节点宿主来说，依赖信息不是运行时临时生成，而是 `Blueprint` 的一部分。
...
- `packages/examples/web-playground/src/tab-panel.ts` 已把第二个 tab 接到 `VIRTUAL_LIST`，通过 scroll viewport + spacer 展示 1000 item 的最小长列表例子。
...
并且这些能力已经可以从 `BlockIR` 经 lowering 生成 `Blueprint` 后被节点宿主 runtime 消费。
